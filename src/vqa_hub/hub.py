...
class Hub:
    def __init__(self,
        ...
        self.ocr_text_min_chars = ocr_text_min_chars
    ...
    def run_vqa(self, image_path: str, question: str) -> HubResult:
        ...
        if "ocr" in decision.path and self.ocr_unit is not None:
            ...
            context = f"Caption: {caption}. On-image text: {ocr_text}." if len(ocr_text) >= self.ocr_text_min_chars else f"Caption: {caption}."
        ...
        # Optional big fallback (requires Ollama + config toggle)
        if self.big_model and "maybe_big" in decision.path:
            if self._low_confidence(answer):
                ...
                big_answer = self.big_model.invoke(prompt).strip()
